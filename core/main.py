#!/usr/bin/env python3
import shutil, sys, os, re, subprocess, core.fetches
...
class ConfigInterlayer:
    ...
    def get_evaluated_layout(self):
        ...
        for f in dir(core.fetches):
            if callable(getattr(core.fetches, f)) and not f.startswith('__'):
                eval_dict[f] = getattr(core.fetches, f)
...
def print_logo_info_line_left_right(logo_line: str, info_line: str, position: str, 
...
def render(config_object: ConfigInterlayer):
    ...
    for logo_line, info_line in zip_longest(logo.splitlines(), info.splitlines(), fillvalue=''):
        ...
        print_logo_info_line_left_right(
